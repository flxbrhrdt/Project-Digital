...
while intro:
    ...
    text1 = font.render(
        "Welcome to Connect 4, Press S to start the game. Use 1,2,3,4,5,6,7 keys to place pieces", True, (background_color))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_7:
                ...
                print('keypress')
            # There's some duplication in the if statements above.
            # Consider using a dictionary that maps between keys and columns:
            #   key_columns = {pygame.K_1: 0, …}
            #   if event.key in key_columns:
            #     return functions.look_through_rows(gameboard.matrix, key_columns[event.key], player)
            print(gameboard.matrix)
